...
def load_everything():
    ...
    # classical model
    clf1 = joblib.load(asset("classical_model.joblib"))
    tfidf = joblib.load(asset("tfidf_vectorizer.joblib"))
    ...
    deberta_mdl = AutoModelForSequenceClassification.from_pretrained(DEBERTA_REPO).eval()
    ...
    class BiLSTMClassifier(nn.Module):
        ...
        def forward(self, x):
            ...
            return self.fc(self.dropout(torch.cat([h_n[0], h_n[1]], dim=1)))

    textcnn = TextCNN(vocab_size, EMBED_DIM)
    textcnn.load_state_dict(torch.load(asset("textcnn.pt"), map_location="cpu"))
    textcnn.eval()

    bilstm = BiLSTMClassifier(vocab_size, EMBED_DIM)
    bilstm.load_state_dict(torch.load(asset("bilstm.pt"), map_location="cpu"))
    bilstm.eval()

    return {
        "clf1": clf1, "tfidf": tfidf,
        "word2idx": word2idx, "PAD_IDX": PAD_IDX,
        "MAX_LEN": MAX_LEN,
...
# load spinner shown only on first load / after waking up
...
def classical_probs(prompt, options):
    ...
    sims  = {L: cosine_similarity(pv, tfidf.transform([options[L]]))[0][0]
             for L in LETTERS}
    ...
    for L in LETTERS:
        ...
        feat = [[sims[L], rank[L], sims[L]/total, srow.max()-sims[L],
...
if st.button("🔍 Predict", type="primary"):
    if not prompt.strip():
        ...
        if exact:
            ...
            st.info("⚡ Exact match found in training data — guaranteed correct")
        else:
